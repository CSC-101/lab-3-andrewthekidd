more = [x + 1 for x in [1,2,3,4]]     # List, in order, the values that x takes at each step. [2, 3, 4, 5]
print(more)                           # What is the value of more at this point? the value of more is now [2, 3, 4, 5]

def square(n:int) -> int:
    return n * n                           # Record, in order of the calls, each value of n and the corresponding return value.
                                           # 1, 2, 3, 4


squares = [square(x) for x in [1,2,3,4]]   # What is the value of squares and how does this relate to the
print()                                    # values recorded above? [1, 4, 9, 16]  the values are multiplied by themselves

def check(n:int) -> bool:
    return n > 2                             # Record, in order of the calls, each value of n and
                                             # the corresponding return value. 0, 1, 2, 3, 4


answer = [x for x in range(5) if check(x)]   # What is the value of answer? [3, 4]
print()

def inc(m:int) -> int:
    return m + 1                             # Record, in order of the calls, each value of m and
                                             # the corresponding return value. m: 4


def check(n:int) -> bool:
    return n > 2                             # Record, in order of the calls, each value of n and
                                             # the corresponding return value. n: 1, 2, 3


answer = [inc(x) for x in range(5) if check(x)]   # What is the value of answer? [4,5]
print("The answer is", answer)